...
public class AlertConfigService {
	...
	public int deleteOneAlertConfig(int alertConfigID) {
		...
		log.debug("deleteAlertflag:" + flag);
	 ...
	 * @return
	 */
	public QueryData queryBypage(CommonQuery query, Integer pageNumber,
			Integer userID, Integer alertPageCount) {
		...
		log.debug("configListSize:" + configList.size());
		...
		totalPage = QueryData.computeTotalPage(totalCount,
	 ...
	 * @return
	 */
	public List<Alert> queryAlertByConfig(Integer alertConfigID,
			CommonQuery query, Integer pageNum) {
		List<Alert> alertList = new ArrayList<Alert>();
		ListQuery queryMap = query.format();
	 ...
	 * @return
	 */
	public QueryData queryAlertBypage(Integer alertConfigID, CommonQuery query,
			Integer pageNumber) {
	...
	public HSSFWorkbook export(List<Alert> alertList) {
		String[] excelHeader = { "IMEI", "IMSI", "日期", "经度", "纬度", "手机型号",
				"APP类型", "是否处理", "推送状态" };
		String sheetName = "告警";
