define(function (require, exports, module){
	...
	var list = {
            ...
            only: function () {
                $('.show_data').delegate('#only', 'click', function () {
                    var id = $(this).closest('li').attr('data-id'),
                        index = $(this).closest('li').attr('data-index');
                        if ($(this).hasClass('active') && $(this).hasClass('btn-danger')) {
                            if (window.polygon.length && window.polygon[index]) {
                                _.each(window.polygon, function (v, i) {
                                    v.show();
                                });
                            }
                            $('.marker').show();
                            ...
                            if (window.polygon.length && window.polygon[index]) {
                                ...
                                if (window.polygon[index]) {
                                    window.polygon[index].show();
                                }
                            }
                            $('.marker').hide();
                            $('.marker' + id).show();
			...
			drop: function () {
				...
				var data_id ;
				show_data.delegate('.drop-warning', 'click', function () {
					data_id = $(this).closest('li').attr('data-id');
					$('#del').one('show.bs.modal', function (event) {
						
					});
					
				});
				$("#delIt").click(function(){
					$$warnning_model.delete_warnning({
						alertConfigID: data_id
					},function () {
			...
			hover: function () {
				...
				show_data.delegate('li', 'mouseover', function () {
					...
					if (window.polygon.length && window.polygon[$(this).attr('data-index')]) {
			...
			add: function () {
                $('.warning_area').on('click', function () {
                    if (window.area) {
                        window.myMap.removeOverlay(window.area);
                        window.area = null;
                        window.longitudeA = null;
                        window.longitudeB = null;
                        window.latitudeA = null;
                        window.latitudeB = null;
                        $('.warning_area')
                            .removeClass('active')
				...
				$("#form_add").click(function(){
                    ...
                    var longitudeA = window.longitudeA || null;
                    var longitudeB = window.longitudeB || null;
                    var latitudeA = window.latitudeA || null;
                    var latitudeB = window.latitudeB || null;

                    if((add_IMEI==""||add_IMEI==null)&&(add_IMSI==""||add_IMSI==null)&&(longitudeA==null||longitudeB==null||latitudeA==null||latitudeB==null)){
						alert("请填写告警条件！");
                    }
                    else{
