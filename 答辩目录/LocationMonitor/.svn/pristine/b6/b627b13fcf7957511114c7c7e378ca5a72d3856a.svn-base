...
public class Kmeans<T> {  
     ...
     * 所有数据列表 
     */  
    private List<T> location = new ArrayList<T>();  
    ...
    public Kmeans(List<T> list, int k) {  
        ...
        initLocation = new ArrayList<T>();  
        for (int i = 0; i < k; i++) {  
            initLocation.add(location.get(i));  
        }  
    }  
    ...
    public List<T>[] comput() {  
        ...
        while (centerchange) {  
            ...
            for (int i = 0; i < location.size(); i++) {  
                ...
                for (int j = 0; j < initLocation.size(); j++) {  
                    T initP = initLocation.get(j);  
                    /* 计算距离 */  
            ...
            for (int i = 0; i < k; i++) {  
                T location_new = findNewCenter(results[i]);  
                T location_old = initLocation.get(i);  
                if (!IsLocationEqual(location_new, location_old)) {  
                    centerchange = true;  
                    initLocation.set(i, location_new);  
                }  
     ...
     * @return 
     */  
    public boolean IsLocationEqual(T p1, T p2) {  
        if (p1 == p2) {  
            return true;  
        }  
        if (p1 == null || p2 == null) {  
